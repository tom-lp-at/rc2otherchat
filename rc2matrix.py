...
default_password = "xxxxyyyySSSdsfkjdsklfjls"
...
def get_token(user):
    ...
    api_params = {"type": "m.login.password","user": user,"password": default_password,"device_id": "rc2m"}
...
def createArgParser():
    ...
    parser.add_argument("-m", type=str, help='Matrix Domain', dest="domainname", default="example.com")
...
def format_date_message(raw):
    #formatted = raw
    #formatted = re.sub("```(.+)```", "<code>\\1</code>", formatted)
    #formatted = re.sub("`(.+)`", "<code>\\1</code>", formatted)
    formatted = markdown.markdown("``` 📆 " + raw + "```") # + "_________________________" 📅
#    formatted = "* " + raw
    api_params = {'msgtype': 'm.text', 'body': raw , # + "_________________________"
            "format": "org.matrix.custom.html",
            "formatted_body": formatted}
...
def format_time_message(raw):
    #formatted = raw
    #formatted = re.sub("```(.+)```", "<code>\\1</code>", formatted)
    #formatted = re.sub("`(.+)`", "<code>\\1</code>", formatted)
    #formatted = markdown.markdown("* " + raw + " Uhr______________")
    formatted = markdown.markdown("` 🕐 " + raw + " o´clock`")
    
#    formatted = "<p align='right'>" + raw + "</p>"
    api_params = {'msgtype': 'm.text', 'body': raw + " o´clock",
            "format": "org.matrix.custom.html",
            "formatted_body": formatted}
...
def invite(api_base, api_headers_admin, tgtroom, tgtuser):
    ...
    if response.status_code != 200:
        ...
        try:
            response = requests.get(api_endpoint, headers=api_headers_as)
            vprint(response.json())
...
#                exit(1)
            ...
            print("Hat nicht geklappt - weiter...")
            
        print("API-Endpoint : " + str(api_endpoint) +" => " + repr(api_headers_as) )
...
if __name__ == '__main__':
    ...
    with open(args.inputs + usersfile, 'r') as jsonfile:
        ...
        for line in jsonfile:
            ...
            api_endpoint = api_base + "_synapse/admin/v2/users/@" + username + ":" + args.domainname
            api_params = {"admin": False, "displayname": displayname, "verify": False}
...
#                exit(1)
            ...
            if "avatarETag" in currentuser:
                try: # try to find the file in the export
                    api_endpoint = api_base + "_matrix/media/v3/upload?user_id=@" + username + ":" + args.domainname
                    ...
                    api_endpoint = api_base + "_synapse/admin/v2/users/@" + username + ":" + args.domainname
                    api_params = {"admin": False, "avatar_url": mxcurl, "verify": False}
                    response = session.put(api_endpoint, json=api_params, headers=api_headers_admin)
                    if response.status_code < 200 or response.status_code > 299: #2xx
...
#                        exit(1)
    ...
    # Get current Tokens for users
    
    for akt_user in users:
        print("Aktueller Benutzer : " + str(akt_user))
        token = get_token(akt_user)
        print("Der Token lautet : " + str(token))
        user_token[akt_user] = token
        time.sleep(2)
    ...
    with open(args.inputs + roomsfile, 'r') as jsonfile:
        ...
        for line in jsonfile:
            ...
            if 'u' in currentroom:
                try:
                    owner_id = "@" + currentroom['u']['username'] + ":" + args.domainname
                    api_endpoint = api_base + "_synapse/admin/v2/users/" + owner_id
                    vprint(api_endpoint)
                    
                    api_headers_user = {"Authorization":"Bearer " + user_token[username]}

                    response = session.get(api_endpoint, headers=api_headers_user)
                    vprint(response.json())
                    if not 'errcode' in response.json():
                        api_endpoint = api_base + "_synapse/admin/v1/users/" + owner_id + "/login"
                        response = session.post(api_endpoint, headers=api_headers_user)
                        vprint(response.json())
            ...
            try:
                ...
                api_params = {"user_id": "@" + currentroom['u']['username'] + ":" + args.domainname}

                api_headers_user = {"Authorization":"Bearer " + user_token[username]}

                response = session.post(api_endpoint, json=api_params, headers=api_headers_user)
                if response.status_code != 200:
            ...
            if "avatarETag" in currentroom:
                try: # try to find the file in the export
                    api_endpoint = api_base + "_matrix/media/v3/upload?user_id=@" + currentroom['u']['username'] + ":" + args.domainname
                    api_params = {'filename': roomids[currentroom['_id']]}

                    api_headers_user = {"Authorization":"Bearer " + user_token[currentroom['u']['username']]}
                    api_headers_file = api_headers_user.copy()
#                    api_headers_file = api_headers_as.copy()
                    # api_headers_file['Content-Type'] = attachment['image_type']
                    ...
                    invite(api_base, api_headers_admin, roomids[currentroom['_id']], '@' + currentroom['u']['username'] + ":" + args.domainname)
                    api_endpoint = api_base + "_matrix/client/v3/rooms/" + roomids[currentroom['_id']] + '/state/m.room.avatar/?user_id=@' + currentroom['u']['username'] + ":" + args.domainname
                    api_params = {"url": mxcurl}


                    response = session.put(api_endpoint, json=api_params, headers=api_headers_user)
    ...
    with open(args.inputs + histfile, 'r') as jsonfile:
        ...
        for line in jsonfile:
            ...
            if currentmsg['rid'] in roomids:
                ...
                tgtuser = "@" + currentmsg['u']['username'] + ":" + args.domainname # tgtuser is the matrix user
                tempdata = currentmsg['ts']['date']
                # tempdata = tempdata.replace('Z','+00:00')
                #dateTimeObj = datetime.fromisoformat(tempdata)
                ...
                if 'attachments' in currentmsg and hasattr(currentmsg['attachments'], '__iter__'):
                    for attachment in currentmsg['attachments']:
                        if 'type' in attachment and attachment['type'] == 'file': # A file
                            ...
                            api_headers_file = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
...
#                            api_headers_file = api_headers_as.copy()
                            ...
                            try: # try to find the file in the export
                                ...
                                if temp_mimetype.startswith("image") or temp_mimetype.startswith("application") or temp_mimetype.startswith("video")  :
                                    ...
                                    api_params['filename'] = api_params['filename'] + "." + extension
                                    ...
                                    print("Neuer Filename : " + api_params['filename'])
                                ...
                                vprint(response.json())
                                if response.status_code != 200: # Upload problem
                                    print("Upload angeblich fehlgeschlagen....")
...
#                                           api_params = {'msgtype': 'm.image', 'body': attachment['title'], 'info': {'mimetype': attachment['image_type'] } ,'url': mxcurl}
#                                        else: # other files

    
                                # ab hier ------------------------------------------------------------------------------------------------
                                if temp_mimetype.startswith("image"):
                                    if "image_dimension" in attachment:
                                        api_params = {'msgtype': 'm.image', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype,'h': attachment['image_dimensions']['height'],'w':attachment['image_dimensions']['width'],'size':attachment['image_size']  }, 'url': mxcurl}
                                    else:
                                        if "image_size" in attachment:
                                            api_params = {'msgtype': 'm.image', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype,'size':attachment['image_size']  }, 'url': mxcurl}
                                        else:
                                            api_params = {'msgtype': 'm.image', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype }, 'url': mxcurl}
                                            
                                elif temp_mimetype.startswith("video"):
                                    api_params = {'msgtype': 'm.video', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype }, 'url': mxcurl}
                                elif temp_mimetype.startswith("audio"):
                                    api_params = {'msgtype': 'm.audio', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype }, 'url': mxcurl}
                                else:
                                    api_params = {'msgtype': 'm.file', 'body': attachment['title'], 'info': {'mimetype': temp_mimetype }, 'url': mxcurl}
                            
                            
                                print("--------------------------------------------------")
                                print("Attachment => " + repr(attachment))
                                print("--------------------------------------------------")
                                #bis hierher--------------------------------------------------------------------------------------
                                print ("for Filename " + localfile + " folgende API-Params " + repr(api_params) )
                            
                                api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
                                response = session.post(api_endpoint, json=api_params, headers=api_headers_user)
                                if response.status_code == 403 and response.json()['errcode'] == 'M_FORBIDDEN': # not in the room
                                    vprint(response.json())
                                    print("Wurde abgelehnt !!! Mit Invite probieren ....")
#                                    api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
                                    
                                    if "not in room" in response.json()['error']:
                                        print("Ist nicht im Raum - mit invite probieren")
                                        invite(api_base, api_headers_as, tgtroom, tgtuser)
...
#                                       else:
                                ...
                                print("OS-Error : " + os.strerror(errno.ENOENT) )
                                api_endpoint = api_base + "_matrix/client/v3/rooms/" + tgtroom + '/send/m.room.message?user_id=' + tgtuser + "&ts=" + str(tgtts) # ts, ?user_id=@_irc_user:example.org
                                api_params = {'msgtype': 'm.text', 'body': "<<< A file named \"" + attachment['title'] + " / " + attachment['title_link'] + "\" was lost during the migration to Matrix >>>"}
                                response = session.post(api_endpoint, json=api_params, headers=api_headers_as)
                                if response.status_code == 403 and response.json()['errcode'] == 'M_FORBIDDEN': # not in the room
                                    api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
...
#                                api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
                ...
                if 'msg' in currentmsg:
                    if currentmsg['msg'] != "" and not finished:
                        api_endpoint = api_base + "_matrix/client/v3/rooms/" + tgtroom + '/send/m.room.message?user_id=' + tgtuser + "&ts=" + str(tgtts) + "&origin_server_ts=" + str(tgtts) + "&sender=" + tgtuser # ts, ?user_id=@_irc_user:example.org


#                        print("-----------------------------------------------------")
#                        print("api_endpoint => " + repr(api_endpoint))
#                        print("api_headers_as => " + repr(api_headers_as))
#                        print("api_params => " + repr(api_params))
#                        print("-----------------------------------------------------")

                        if currentmsg['u']['username'] in user_token:
                            api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
                            date_ts = date.fromtimestamp(tgtts)
                            date_temp = str(date_ts)
                            time_temp = datetime.fromtimestamp(tgtts).strftime("%H:%M")
                            
                            # If date changes, print the new date in messages
                            if old_date != date_temp:
                                date_params = format_date_message("Date " + date_temp)
                                old_date = date_temp
                                old_time = 0
                                reponse = session.post(api_endpoint, json=date_params, headers=api_headers_user)

                            # calcuate a diff from 10 Minutes. If the message is newer then 10 Minutes after the last message, print the new time...
                            new_time = tgtts - 600
                            if new_time > old_time:
                                date_params = format_time_message(time_temp)
                                old_time = tgtts
                                reponse = session.post(api_endpoint, json=date_params, headers=api_headers_user)
                            else:
                                print("Der tgtts = " + str(tgtts) + " und old_time = " + str(old_time))
    #                        response = session.post(api_endpoint, json=api_params, headers=api_headers_as)
...
#                                api_headers_user = {"Authorization":"Bearer " + user_token[currentmsg['u']['username']]}
                            ...
                            print("Nachricht von User " + currentmsg['u']['username'] + " hat keinen Token - mal weiter....")
                            continue
                    ...
                    if (currentmsg['reactions'] != None ):
                        for reaction in currentmsg['reactions']:
                            ...
                            for username in currentmsg['reactions'][reaction]['usernames']:
                                tgtusername = "@" + username + ":" + args.domainname
